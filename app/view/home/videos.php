...
$user=$data['user'];
@$form=$data['form'];
@$query=Input::get(array('art_category_id','artist_category_id','genre_id','feature_v','search'));
$count=$data->count;
$tquery=Input::get();
if (!isset($tquery['art_category_id']) && (isset($tquery['genre_id']) || isset($tquery['artist_category_id']) && !isset($tquery['feature_v']))) {
	unset($tquery['genre_id']);
	unset($tquery['artist_category_id']);
	unset($tquery['feature_v']);
...
View::make('include/v1_header',array('title'=>$title,'headerKey'=>'videos','head'=>$head));
?>
<style type="text/css">
	...
	<div class="row filterbox">
		...
		<div class="col s9">
			...
			<div class="multiple-select-box">
				...
				<form class="filter_tag" style="display:none" action="/videos">
		...
		<form class="filter_form">
			<div class="col s3 card" id="filter" style="position:fixed;top:62px;min-height:90%">
				<div class="allresult clearfix">
					<span style="font-size:22px" class="left"><h1 style="display:inline;margin:0;padding:0;font-size:22px">Videos </h1>(<count id="vcount">0</count> Results)</span>
				</div>
				<div class="allresult clearfix">
					<h5 class="left">Filter by</h5>
					<a href="/videos"><span class="right btnclear">Clear all <i class="fa fa-times"></i></span></a>
				</div>
				...
				<div class="filtersection-inner">
					<div class="filter-type">
						<span class="checkebox">
							<input type="checkbox" class="filtecheckbox" id="filter-vid" checked>
							<label for="filter-vid">Video Type</label>
						</span>
					</div>
					...
					<div class="filter-content" style="display:block;">
						<div class="filter-inner clearfix">
							<div class="left">
								<input class="with-gap filter-input" <?=$query['feature_v']==1?'checked="checked"':''?> name="feature_v" value="1" type="radio" id="feature_v_1">
								<label for="feature_v_1">Featured Videos</label>
							</div>
						</div>
						<div class="filter-inner clearfix">
							<div class="left">
								<input class="with-gap filter-input" <?=$query['feature_v']==2?'checked="checked"':''?> name="feature_v" value="2" type="radio" id="feature_v_2">
								<label for="feature_v_2">Other videos</label>
							</div>
						</div>
					</div>
				</div>
...
<script type="text/javascript">
	...
	$(".filter_form").submit(function(e){
		...
		window.location.href='/videos?'+query;
	});
...
<script type="text/javascript">
	...
	$(document).on('click','#load_more',function(){
		...
		$.ajax({
			url: '<?=API_URL?>/video/feature?page='+page+'<?=Input::get('type')?"&type=past":""?>&'+query_token+'&'+'<?=http_build_query($query)?>',
		}).done(function(data){
	...
	function appendVideos(v){
		...
		if(typeof(v.json.items[0])==='undefined'){
			return '';
		}
